...
def test_thumbnail_cache_expires_entries_after_ttl() -> None:
    ...
    cache.set("dropbox", "photo-1", thumbnail)

    assert cache.get("dropbox", "photo-1") is None
...
def test_thumbnail_cache_evicts_oldest_live_entry_when_full() -> None:
    ...
    cache.set("dropbox", "photo-1", Thumbnail(b"first", "image/jpeg"))
    cache.set("dropbox", "photo-2", Thumbnail(b"second", "image/jpeg"))

    assert cache.get("dropbox", "photo-1") is None
    assert cache.get("dropbox", "photo-2") == Thumbnail(b"second", "image/jpeg")
